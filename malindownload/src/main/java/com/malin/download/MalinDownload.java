...
import com.malin.download.entity.DownloadEvent;
...
import com.malin.download.entity.MultiMission;
import com.malin.download.entity.SingleMission;
import com.malin.download.function.DownloadHelper;
import com.malin.download.function.DownloadService;
...
public class MalinDownload {
    ...
    private volatile static MalinDownload instance;
    ...
    private MalinDownload(Context context) {
    ...
    /**
     * Return MalinDownload Instance
     *
     * @param context context
     * @return MalinDownload
     */
    public static MalinDownload getInstance(Context context) {
        if (instance == null) {
            synchronized (MalinDownload.class) {
                if (instance == null) {
                    instance = new MalinDownload(context);
                }
            }
        }
        return instance;
     ...
     * @return instance.
     */
    public MalinDownload defaultSavePath(String savePath) {
     ...
     * @return instance.
     */
    public MalinDownload retrofit(Retrofit retrofit) {
     ...
     * @return instance
     */
    public MalinDownload maxThread(int max) {
     ...
     * @return instance
     */
    public MalinDownload maxRetryCount(int max) {
     ...
     * @return instance
     */
    public MalinDownload maxDownloadNumber(int max) {
    ...
    public Observable<?> serviceDownload(final DownloadBean bean) {
        return createGeneralObservable(new GeneralObservableCallback() {
            @Override
            public void call() throws InterruptedException {
                downloadService.addDownloadMission(new SingleMission(MalinDownload.this, bean));
            }
        }).observeOn(AndroidSchedulers.mainThread());
    }
    ...
    public Observable<?> serviceMultiDownload(final List<DownloadBean> beans, final String missionId) {
        return createGeneralObservable(new GeneralObservableCallback() {
            @Override
            public void call() throws InterruptedException {
                downloadService.addDownloadMission(new MultiMission(MalinDownload.this, missionId, beans));
            }
        }).observeOn(AndroidSchedulers.mainThread());
    }
    ...
    public boolean isExistDownloadRecord(String url){
        return downloadHelper.isExistDownloadRecord(url);
    }
